...
async def start_command(client: Client, message: Message):
    ...
    if bot.db:
        if not await bot.db.is_user_exist(user_id):
            await bot.db.add_user(user_id)
    
    # Check if banned
    if bot.db and await bot.db.is_user_banned(user_id):
...
"""
    ...
    try_again_link = f"https://t.me/{bot.username}?start={file_param}"
    buttons.append([
...
async def send_file(client: Client, message: Message, file_param: str):
    ...
    if sent_messages and AUTO_DELETE_TIME > 0:
        link = f"https://t.me/{bot.username}?start={file_param}"
